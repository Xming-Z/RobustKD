...
class PyramidNet(nn.Module):
    ...
    def forward_feature(self, x,  mask = None, feature = None, preReLU=True):
        x = self.conv1(x)
        x = self.bn1(x)
        ...
        if mask == None:
            ...
            # x = self.layer1(x)
            # new_feat1 = x
            # feat1 = x + mask[0] * feature[0]
            #
            # x = self.layer2(feat1)
            # feat2 = x + mask[-2] * feature[-2]
            #
            # x = self.layer3(feat2)
            # feat3 = x + mask[-1] * feature[-1]

            feat1 = self.layer1(x)
            x = feat1 + mask[0] * feature[0]

            feat2 = self.layer2(x)
            x = feat2 + mask[-2] * feature[-2]

            feat3 = self.layer3(x)
            x = feat3 + mask[-1] * feature[-1]
